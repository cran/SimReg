...
geom_known_prior::geom_known_prior(
	...
	Rcpp::NumericVector in_baseline_to_term_similarities,
	double in_rate
) {
	row_is_column_anc = in_row_is_column_anc;
	baseline_to_term_similarities = in_baseline_to_term_similarities;
	rate = in_rate;
	mean_baseline_sim = 0.0;
	int n = baseline_to_term_similarities.length();
	for (int i = 0; i < n; i++)
		mean_baseline_sim += baseline_to_term_similarities[i];
	mean_baseline_sim /= (double)n;
}
...
double geom_known_prior::operator()(Rcpp::IntegerVector delta) { 
	int baseline_sim = 0.0;
	int N = delta.length();
	for (int i = 0; i < N; i++)
		baseline_sim += baseline_to_term_similarities(delta[i]);
	baseline_sim = baseline_sim / (double)N;

	Rcpp::LogicalVector is_leaf = hpo_leaves(row_is_column_anc, delta);
	...
	return (double)num_leaves * log(rate) + log(normalise_by) + log(baseline_sim) - log(mean_baseline_sim) - log((double)num_leaf_set_reps(
		num_leaves,
		num_exclusive_ancs,
		delta.length()
	));
